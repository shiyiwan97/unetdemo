...
class NewTrain:
    ...
    def train(self, config):
        ...
        loss_function2 = LossUtil.CrossEntropyLoss(255).get_loss()
        # 5.定义优化器
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=7, shuffle=True, drop_last=True)
        ...
        while epoch < 200:
            ...
            for i, (image, segment_image) in enumerate(tqdm.tqdm(train_dataloader)):
                ...
                optimizer.zero_grad()
                loss.backward(cuda_launch_blocking=True)
                optimizer.step()
                train_loss_sum += loss.item()
                print(f'{epoch}-{i}-train_loss_CEL===>>' + str(loss_function2(out_image, segment_image.long()).item()))
            torch.save(unet.state_dict(), r'.\weight\baseline\dev_01\weight_' + str(epoch) + r'.pth')
